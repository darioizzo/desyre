...
void perform_speed_test(unsigned n_vars, unsigned n_cons, unsigned length, unsigned N, unsigned seed)
{
    ...
    std::vector<double> d0phenotype, d1phenotype;
    std::vector<double> ddphenotype;
    ...
    for (decltype(vars.size()) i = 0u; i < vars.size(); ++i) {
        ...
        for (auto j = 0u; j < n_vars + n_cons; ++j) {
            ex.dphenotype(d0phenotype, genotype, phenotype, j);
            for (auto k = 0u; k <= j; ++k) {
                ex.dphenotype(d1phenotype, genotype, phenotype, k);
                ex.ddphenotype(ddphenotype, genotype, phenotype, d0phenotype, d1phenotype);
            }
        }
    }
